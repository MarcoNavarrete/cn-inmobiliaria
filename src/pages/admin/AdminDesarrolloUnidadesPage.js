  ...
  useEffect(() => {
    ...
    fetch(resolveApiAssetUrl(planoForm.svgUrl), { signal: controller.signal })
      ...
      .then((text) => {
        ...
        setSvgIds(Array.from(doc.querySelectorAll('[id]')).map((element) => element.id));
      })
      .catch((err) => {
  ...
  return (
    <main className="admin-unidades">
      ...
      <section className="admin-unidades-card">
        <div className="admin-unidades-card-head">
          <h2>Listado</h2>
          <span className="admin-unidades-pill">{unidades.length}</span>
        </div>
                ...
                {unidades.map((unidad) => (
                  <tr key={unidad.id}>
                    ...
                    <td data-label="SVG">
                      ...
                      {svgIds.length > 0 && !svgIds.includes(unidad.svgElementId) ? (
